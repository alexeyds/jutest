...
import { presentErrorMessage, presentSourceDetails, replaceCwd } from "./error-presenters";
  ...
  async _reportFailedTest(test, testNumber) {
    ...
    let section = buildNumberedTestSection(test, testNumber, s => {
      s.addSection(s => {
        ...
        let testLocation = presentTestLocation(test);
        if (testLocation) {
          s.addLine();
...
function presentTestLocation(test) {
  let { lineNumber, file } = test.definitionLocation;
  let result = '';
