...
import { ErrorSourceTracer } from "./error-source-tracer";
...
export async function presentSourceDetails(error, runtimeConfig) {
  if (error?.stack) {  
    let sourceTracer = new ErrorSourceTracer(error, runtimeConfig);
...
function replaceCwd(path) {
  return path.replace('file://', '').replace(process.cwd(), ".");
}
