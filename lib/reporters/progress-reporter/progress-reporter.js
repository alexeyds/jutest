...
export class ProgressReporter {
  ...
  reportTestResult(testSummary) {
    ...
    if (executionResult.status === TestExecutionStatuses.Passed) {
      process.stderr.write(chalk.green('.'));
    } else if (executionResult.status === TestExecutionStatuses.Skipped) {
  ...
  async reportSummary(runSummary) {
    ...
    let color = runSummary.success ? chalk.green : chalk.red;
