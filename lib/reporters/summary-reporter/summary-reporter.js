...
export class SummaryReporter {
  ...
  finishReporting(runSummaries) {
    ...
    let section = buildTextSection(s => {
      ...
      if (skippedCount > 0) {
        let pluralForm = skippedCount === 1 ? 'test was' : 'tests were';
        s.addLine(chalk.yellow(`${skippedCount} ${pluralForm} skipped`));
      }

      let color = failedCount > 0 ? 'red' : 'green';
      s.addLine(chalk[color](`Total: ${totalCount}, Failed: ${failedCount}`));
