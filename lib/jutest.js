...
export async function autoRun({
  ...
  testRunner.on('test-end', test => testReporter.reportTestResult(test));
  testRunner.on('test-skip', test => testReporter.reportTestResult(test));
  testRunner.on('run-end', runSummary => testReporter.reportSummary(runSummary));
