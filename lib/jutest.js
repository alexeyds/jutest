...
import { ProgressReporter, SummaryReporter, FailedTestsReporter, RerunnableLocationsReporter, ReporterConfig } from 'reporters';
...
export async function autoRun({
  ...
  let reporters = reporterClasses.map(ReporterClass => {
    return ReporterClass.initializeReporter(reporterConfig, runner.eventEmitter);
  });
