...
let testRunner = new TestRunner({ testsContainer });
...
export async function autoRun({
  ...
  if (fileWithLocation) {
    result = await testRunner.runAtFileLocation(fileWithLocation);
  } else {
    result = await testRunner.runAll();
  }
