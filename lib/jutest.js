...
export async function autoRun({
  ...
  testRunner.on(Events.TestEnd, test => testReporter.reportTestResult(test));
  testRunner.on(Events.TestSkip, test => testReporter.reportTestResult(test));
  testRunner.on(Events.RunEnd, runSummary => testReporter.reportSummary(runSummary));
