...
import { TestRunner, RuntimeEventEmitter, RuntimeContext } from "runtime";
...
export async function autoRun({
  ...
  runtimeEventEmitter.on(Events.TestEnd, test => testReporter.reportTestResult(test));
  runtimeEventEmitter.on(Events.TestSkip, test => testReporter.reportTestResult(test));
  runtimeEventEmitter.on(Events.RunEnd, runSummary => testReporter.reportSummary(runSummary));
