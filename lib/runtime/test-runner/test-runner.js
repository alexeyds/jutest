...
export class TestRunner {
  ...
  async _run(specs) {
    ...
    this._emit(Events.RunEnd, runSummary);
  ...
  async _runSuite(suite) {
    ...
    this._emit(Events.SuiteStart, suiteSummary);
    await this._runSpecs(await suite.composeSpecs());
    this._emit(Events.SuiteEnd, suiteSummary);
  }
  ...
  async _runTest(test) {
    ...
    if (test.skipped) {
      this._emit(Events.TestSkip, initialSummary);
    } else {    
      this._emit(Events.TestStart, initialSummary);
      await test.run();

      let resultSummary = this._summary.addTestResult(test);
      this._emit(Events.TestEnd, resultSummary);
    }
  }
  ...
  _emit(...args) {
    return this._runtimeContext.runtimeEventEmitter.emit(...args);
  }
}
