...
export default function throws(expression, matcher) {
  ...
  if (success) {
    return Assertion.create({
      ...
      operator: 'throws',
      actual: undefined,
      buildFailureMessage: () => noErrorRaisedMessage({matcher}),
      expected: matcher
    });
  } else {
    return Assertion.create({
      ...
      operator: 'throws',
      buildFailureMessage: () => wrongErrorRaisedMessage({matcher, error}),
      actual: error,
      expected: matcher
    });
  }
...
function wrongErrorRaisedMessage({error, matcher}) {
  return (
    `Expected ${inspectMatcher(matcher)}, got: ${inspect(error)}` +
    `\n` +
    `\n` +
    `(operator: throws)`
  );
